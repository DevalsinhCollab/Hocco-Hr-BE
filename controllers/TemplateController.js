...
exports.getTemplates = async (req, res) => {
  try {
    ...
    if (["AHR", "HR"].includes(userData.userType)) {
      findObject = { company: new mongoose.Types.ObjectId(userData.company) };
    } else {
      findObject = { company: { $exists: false } };
    }
    ...
    if (["AHR", "HR"].includes(userData.userType)) {
      templates = await TemplateSchema.find(findObject).sort({ createdAt: -1 })
        .populate("company")
        .skip(skip)
        .limit(pageSize)
        .lean()
      ...
      templates = await TemplateSchema.find(findObject).skip(skip)
        .limit(pageSize)
        .lean()
        .exec();;
    }

    return res.status(200).json({ data: templates, success: true });
  } catch (error) {
    return res.status(400).json(error);
  }
};
...
//     return pdfBuffer;
//   } catch (error) {
//     console.log(error, "convertHtmlToPdf error===============");
//   }
// };
...
const mergeStampAndBase64ToPdf = async (base64Buffer) => {
  try {
    ...
    console.log(error, "mergeStampAndBase64ToPdf error===============");
  }
};
...
exports.htmlToPdf = async (req, res, funcData) => {
  try {
    ...
    const findCustomerData = await Customer.findOne({
...
exports.getTemplateById = async (req, res) => {
  try {
    ...
    let templateData = await TemplateSchema.findById(id).lean().exec();
